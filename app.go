...
func (game *Game) CanReroll(diceIndexes []int64) error {
...
func (game *Game) Reroll(diceIndexes []int64) {
	game.hasHungerCrit = false;
	game.hasHungerFail = false;
	game.successes = 0;
	game.critSuccesses = 0;
...
func main() {
	...
	for (scanner.Scan()) {
		...
		if (dicesInput[0] == "r") {
			diceIndexes, err := toInts(dicesInput[1:]);
			if err != nil {
				fmt.Println(err.Error());
				continue;
			}

			if err := game.CanReroll(diceIndexes); err != nil {
				fmt.Println(err.Error());
				continue;
			}
			
			game.Reroll(diceIndexes);
			fmt.Println(game);
